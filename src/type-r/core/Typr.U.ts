...
class P {
  public static moveTo(p: any, x: number, y: number) {
    p.cmds.push('M');
    p.crds.push(x, y);
  }

  public static lineTo(p: any, x: number, y: number) {
    p.cmds.push('L');
    p.crds.push(x, y);
  }

  public static curveTo(p: any, a: number, b: number, c: number, d: number, e: number, f: number) {
    p.cmds.push('C');
    p.crds.push(a, b, c, d, e, f);
  }

  public static qcurveTo(p: any, a: number, b: number, c: number, d: number) {
    p.cmds.push('Q');
    p.crds.push(a, b, c, d);
  }

  public static closePath(p: any) {
    p.cmds.push('Z');
  }
}
...
export class TyprU {
  public static codeToGlyph(font: any, code: number) {
    let cmap = font.cmap;
    ...
    else { throw Error('unknown cmap table format ' + tab.format) }
  }

  public static glyphToPath(font: any, gid: number): any {
    let path = { cmds: [], crds: [] };
  ...
  public static _drawGlyf(gid: number, font: any, path: any): any {
  ...
  public static _compoGlyph(gl: any, font: any, p: any) {
  ...
  public static _simpleGlyph(gl: any, p: any): any {
  ...
  public static _compGlyph(gl: any, font: any, p: any): any {
    for (let j = 0; j < gl.parts.length; j++) {
      let path = { cmds: [], crds: [] };
      let prt = gl.parts[j];
      TyprU._drawGlyf(prt.glyphIndex, font, path);
  ...
  public static _getGlyphClass(g: any, cd: any): any {
    let intr = Core.Lctf.getInterval(cd, g);
    return intr === -1 ? 0 : cd[intr + 2];
  }

  public static getPairAdjustment(font: any, g1: any, g2: any): any {
    if (font.GPOS) {
      let ltab = null;
      for (let i = 0; i < font.GPOS.featureList.length; i++) {
        let fl = font.GPOS.featureList[i];
        if (fl.tag === 'kern') for (let j = 0; j < fl.tab.length; j++) if (font.GPOS.lookupList[fl.tab[j]].ltype === 2) ltab = font.GPOS.lookupList[fl.tab[j]];
      }
      if (ltab) {
        let adjv = 0;
  ...
  public static stringToGlyphs(font: any, str: string): any {
    let gls = [];
  ...
  public static _applyType1(gls: number[], ci: number, tab: any): any {
    let gl = gls[ci];
    for (let j = 0; j < tab.tabs.length; j++) {
      let ttab = tab.tabs[j];
  ...
  public static glyphsToPath(font: any, gls: any, clr: any): any {
    // gls = gls.reverse();//gls.slice(0,12).concat(gls.slice(12).reverse());

    let tpath: any = { cmds: [], crds: [] };
    let x = 0;
  ...
  public static pathToSVG(path: any, prec?: number): any {
    if (prec === undefined) prec = 5;
    let out = [];
    let co = 0;
    let lmap: Record<string, number> = { 'M': 2, 'L': 2, 'Q': 4, 'C': 6 };
    for (let i = 0; i < path.cmds.length; i++) {
      let cmd = path.cmds[i];
      let cn = co + (lmap[cmd] ? lmap[cmd] : 0);
      out.push(cmd);
      while (co < cn) {
        let c = path.crds[co++];
  ...
  public static pathToContext(path: any, ctx: any): any {
    let c = 0;
    let crds = path.crds;
  ...
  public static _drawCFF(cmds: any, state: any, font: any, p: any) {
    let stack = state.stack;
    let nStems = state.nStems; let haveWidth = state.haveWidth; let width = state.width;
    let open = state.open;
    let i = 0;
    let x = state.x;
    let y = state.y;
    ...
    while (i < cmds.length)
    {
      ...
      else if ((String(v)).charAt(0) === 'o') { console.log('Unknown operation: ' + v, cmds); throw v }
      else { stack.push(v) }
    }
    // console.log(cmds);
    state.x = x; state.y = y; state.nStems = nStems; state.haveWidth = haveWidth; state.width = width; state.open = open;
