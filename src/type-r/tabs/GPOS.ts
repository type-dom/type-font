...
export class GPOS {
  ...
  public static subt(data: Uint8Array, ltype: number, offset: number): any {
    if (ltype !== 2) {
      return null;
    }
    ...
    offset += 2;
    let covOff = Bin.readUshort(data, offset);
    offset += 2;
    ...
    if (tab.format === 1) {
      ...
      for (let i = 0; i < count; i++) {
        let psoff = Bin.readUshort(data, offset);
        offset += 2;
        psoff += offset0;
        let pvcount = Bin.readUshort(data, psoff);
        psoff += 2;
        let arr = [];
        for (let j = 0; j < pvcount; j++) {
          let gid2 = Bin.readUshort(data, psoff);
          psoff += 2;
          let value1;
          let value2;
          if (tab.valFmt1 !== 0) {
            value1 = Lctf.readValueRecord(data, psoff, tab.valFmt1);
            psoff += ones1 * 2;
          }
          if (tab.valFmt2 !== 0) {
            value2 = Lctf.readValueRecord(data, psoff, tab.valFmt2);
            psoff += ones2 * 2;
          }
    ...
    if (tab.format === 2) {
      let classDef1 = Bin.readUshort(data, offset);
      offset += 2;
      let classDef2 = Bin.readUshort(data, offset);
      offset += 2;
      let class1Count = Bin.readUshort(data, offset);
      offset += 2;
      let class2Count = Bin.readUshort(data, offset);
      offset += 2;
      ...
      for (let i = 0; i < class1Count; i++) {
        ...
        for (let j = 0; j < class2Count; j++) {
          ...
          let value2 = null;
          if (tab.valFmt1 !== 0) {
            value1 = Lctf.readValueRecord(data, offset, tab.valFmt1);
            offset += ones1 * 2;
          }
          if (tab.valFmt2 !== 0) {
            value2 = Lctf.readValueRecord(data, offset, tab.valFmt2);
            offset += ones2 * 2;
          }
